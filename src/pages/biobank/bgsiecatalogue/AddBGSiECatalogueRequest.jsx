...
import { fetchBioSampleAvailability, fetchParentSamples } from '../../../actions';
...
class AddBGSiECatalogueRequest extends React.Component {
  emitUp = () => {
    try {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      isExistingRequest: !!(selectedRequest && selectedRequest.nbgsiecatrequestcode),
  ...
  applyComboChange = (comboData, fieldName, forceClearDependents = false) => {
    ...
    if (fieldName === 'nbioprojectcode') {
      ...
      const clearThenFetchSites = () => {
        ...
        rsapi()
          .post('bgsiecatalogue/getSiteComboForProject', { nselectedprojectcode: nproject, userinfo })
          .then(response => {
    ...
    if (fieldName === 'nsitecode') {
      const fetchProducts = () => {
        ...
        if (nproject > 0 && nsite > 0) {
          ...
          this.props.dispatch(initRequest(true));
          rsapi()
            .post('bgsiecatalogue/getBioSampleTypeCombo', {
  ...
  onFilterAvailability = () => {
    ...
    if (reqType === RequestFormType.BIO_SAMPLE_BASED) {
      ...
      this.props.dispatch(fetchBioSampleAvailability(payload));
    } else if (reqType === RequestFormType.PARENT_SAMPLE_BASED) {
      this.setState({ suppressHydrateParentSamples: false });
      this.props.dispatch(fetchParentSamples(payload));
    } else {
...
export default connect(mapStateToProps)(injectIntl(AddBGSiECatalogueRequest));