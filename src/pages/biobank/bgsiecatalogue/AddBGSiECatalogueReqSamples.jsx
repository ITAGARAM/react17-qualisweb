...
class AddBGSiECatalogueReqSamples extends React.Component {
  constructor(props) {
    super(props);
    ...
    this.state = {
      ...
      currentRow: {
        ...
        nbgsiecatdetailcode: sr.nbgsiecatdetailcode ?? null,
      },
    ...
    if ((sr.nbgsiecatdetailcode || sr.isEdit) && sr.sparentsamplecode) {
  ...
  isEditMode = () => {
    ...
    return Boolean(sr.nbgsiecatdetailcode || sr.nbgSiecatDetailCode || sr.isEdit);
  };
  ...
  buildFilterPayload = () => {
    ...
    const { selectedRecord = {}, selectedBioBGSiECatalogueRequest = {}, Login } = this.props;
    const mode = (this.props.mode || '').toLowerCase();


    const isEditOperation = this.isEditMode() ? 3 : 4;
    const nbgsiecatdetailcode = (this.state.currentRow?.nbgsiecatdetailcode ?? this.props.selectedRecord?.nbgsiecatdetailcode ?? -1);
    const nproductcode =
    ...
    const resolvedSiteCode =
      ...
      selectedBioBGSiECatalogueRequest?.nreceiversitecode ??
      null;
    ...
    return {
      ...
      nbgsiecatrequestcode:
        selectedBioBGSiECatalogueRequest?.nbgsiecatrequestcode ??
        selectedRecord?.nbgsiecatrequestcode ??
        -1,
      nbioprojectcode:
        selectedRecord?.nbioprojectcode ??
        selectedRecord?.nselectedbioprojectcode ??
        selectedBioBGSiECatalogueRequest?.nbioprojectcode,
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (prevProps.selectedRecord !== this.props.selectedRecord) {
      ...
      const hasMeaning = Boolean(
        sr.nbgsiecatdetailcode || sr.nbgSiecatDetailCode || sr.isEdit ||
      ...
      if (hasMeaning) {
        ...
        if (
          (sr.nbgsiecatdetailcode || sr.isEdit) &&
    ...
    try {
      const prevList = (prevProps.Login && prevProps.Login.masterData && prevProps.Login.masterData.lstBioBGSiECatalogueDetails) || [];
      const currList = (this.props.Login && this.props.Login.masterData && this.props.Login.masterData.lstBioBGSiECatalogueDetails) || [];
  ...
  render() {
    const {
      ...
      disableTypeSelect,
      sampleTypeList = [],
      selectedBioBGSiECatalogueRequest = {},
    } = this.props;
        ...
        <Row className="mb-2">
          <Col md={4}>
            <FormInput
              ...
              value={selectedBioBGSiECatalogueRequest?.sreqformtypename || ''}
              readOnly
            />
          </Col>
          <Col md={4}>
            <FormInput
              ...
              name="project"
              value={selectedBioBGSiECatalogueRequest?.sprojecttitles || ''}
              readOnly
            />
          </Col>
          <Col md={4}>
            <FormInput
              ...
              name="site"
              value={selectedBioBGSiECatalogueRequest?.sreceivingsitename || ''}
              readOnly
            />
          </Col>
        </Row>
...
export default connect(mapStateToProps)(AddBGSiECatalogueReqSamples);
