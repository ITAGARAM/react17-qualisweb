...
class AddThirdPartyReturn extends React.Component {
    constructor(props) {
        super(props)
        const dataState = {
            skip: 0,
        ...
        this.state = {
            ...
            lstThirdPartyFormAcceptanceDetails: []
        }
    }
    ...
    render() {
        ...
        let lstThirdPartyFormAcceptanceDetails = this.state.selectedRecord?.lstThirdPartyFormAcceptanceDetails;
        ...
        return (
            <>
                    ...
                    {this.props?.operation !== "update" &&
                        <Col md={6}>
                            <FormSelectSearch
                                ...
                                isSearchable={true}
                                name={"nbiothirdpartyformacceptancecode"}
                                isDisabled={false}
                                placeholder={this.props.intl.formatMessage({ id: "IDS_SELECTRECORD" })}
                                isMandatory={true}
                                options={this.state.lstFormNumberDetails}
                                optionId='nbiothirdpartyformacceptancecode'
                                optionValue='sformnumber'
                                value={this.state.selectedRecord ? this.state.selectedRecord.nbiothirdpartyformacceptancecode : ""}
                                onChange={(event) => this.onComboChange(event, 'nbiothirdpartyformacceptancecode')}
                    ...
                    {this.props?.operation === "update" &&
                        <Col md={6}>
                            <FormSelectSearch
                                formLabel={this.props.intl.formatMessage({ id: "IDS_FORMNUMBER" })}
                                isSearchable={true}
                                name={"nbiothirdpartyreturncode"}
                                isDisabled={true}
                                placeholder={this.props.intl.formatMessage({ id: "IDS_SELECTRECORD" })}
                                isMandatory={true}
                                options={this.state.lstFormNumberDetails}
                                optionId='nbiothirdpartyreturncode'
                                optionValue='sthirdpartyreturnformnumber'
                                value={this.state.selectedRecord ? this.state.selectedRecord.nbiothirdpartyreturncode : ""}
                                onChange={(event) => this.onComboChange(event, 'nbiothirdpartyreturncode')}
                                closeMenuOnSelect={true}
                                alphabeticalSort={true}
                            />
                        </Col>
                ...
                <Row>
                    {lstThirdPartyFormAcceptanceDetails && lstThirdPartyFormAcceptanceDetails.length > 0 && (
                        <Row className="mt-4">
                            <Col md={12}>
                                <div style={{ overflowX: 'auto' }}>
                                    <Table bordered hover size="sm" className="aliquot-table" style={{ tableLayout: "fixed" }}>
                                        ...
                                        <tbody>
                                            {lstThirdPartyFormAcceptanceDetails && lstThirdPartyFormAcceptanceDetails.length > 0
                                                && lstThirdPartyFormAcceptanceDetails.map((item, index) => (
                                                    <tr key={index}>
                                                        <td>{index + 1}</td>
    ...
    onAliquotVolumeChange = (index, value) => {
        ...
        const updatedList = [...this.state.lstThirdPartyFormAcceptanceDetails];
        updatedList[index].sreturnvolume = sanitizedValue;
        let selectedRecord = this.state.selectedRecord;
        selectedRecord["lstThirdPartyFormAcceptanceDetails"] = updatedList;
        this.setState({ lstThirdPartyFormAcceptanceDetails: updatedList, selectedRecord });
    ...
    onDeleteAliquot = (index) => {
        ...
        let selectedRecord = this.state.selectedRecord;
        selectedRecord["lstThirdPartyFormAcceptanceDetails"] = updatedList;
        const lstThirdPartyFormAcceptanceDetails = updatedList;
        this.setState({ lstThirdPartyFormAcceptanceDetails, selectedRecord });
        this.props.childDataChange(selectedRecord);
    };

    onComboChange = (event, field) => {
        let selectedRecord = this.state.selectedRecord
        if (field === 'nbiothirdpartyformacceptancecode') {
            rsapi().post("biothirdpartyreturn/getThirdPartyFormAcceptanceDetails", { 'userinfo': this.props.Login.userInfo, 'nbiothirdpartyformacceptancecode': event.value || -1 })
                .then(response => {
                    ...
                    let lstThirdPartyFormAcceptanceDetails = response.data.lstThirdPartyFormAcceptanceDetails || [];
                    selectedRecord["lstThirdPartyFormAcceptanceDetails"] = lstThirdPartyFormAcceptanceDetails;
                    this.setState({ selectedRecord, lstThirdPartyFormAcceptanceDetails });
                    this.props.childDataChange(selectedRecord);
                })
                .catch(error => {
...
export default connect(mapStateToProps, {})(injectIntl(AddThirdPartyReturn));