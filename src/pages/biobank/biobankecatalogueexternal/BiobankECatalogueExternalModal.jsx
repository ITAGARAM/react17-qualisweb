...
// const ColumnsSVG = ({ title, categories = [], series = [], stacked = true }) => {
//   const vbW = 960, vbH = 420;
//   const pad = { top: 24, right: 16, bottom: 70, left: 56 };
...
//   const catBand = Math.min(120, plotW / nCats);
//   const innerGap = 0.18;
//   const groupW = stacked ? Math.min(60, catBand * 0.4) : Math.min(60, catBand * (1 - innerGap));
...
//   const legend = series.map((s, i) => ({ label: s.name || `Series ${i+1}`, color: PALETTE[i % PALETTE.length] }));
...
//         <svg width="100%" height="100%" viewBox={`0 0 ${vbW} ${vbH}`} preserveAspectRatio="xMidYMid meet" role="img" aria-label={title}>
//           {ticks.map((t, i) => (
//             <g key={`tick-${i}`}>
//               <line x1={pad.left} y1={y(t)} x2={vbW - pad.right} y2={y(t)} stroke="#e9ecef" strokeWidth="1" />
//               <text x={pad.left - 10} y={y(t)} textAnchor="end" dominantBaseline="central" fontSize="12" fill="#6c757d">{t}</text>
//             </g>
//           ))}
...
//               return (
//                 <g key={`cat-${i}`}>
//                   <g transform={`translate(${cx(i)}, ${vbH - pad.bottom + 12}) rotate(-45)`}>
//                     <text textAnchor="end" fontSize="12" fill="#212529">{String(cat)}</text>
//                   </g>
//                   {series.map((s, si) => {
...
//                     yTop = yy;
//                     return (
//                       <rect key={`seg-${si}`} x={left + (groupW - barW)/2} y={yy} width={barW} height={h}
...
//           })}
//                   <text x={pad.left + plotW / 2} y={vbH - 20} textAnchor="middle" fontSize="14" fontWeight="bold" fill="#212529">{this.props.intl.formatMessage({ id: "IDS_BIOSAMPLETYPE" })}</text>
//           <text transform={`rotate(-90)`} x={-(pad.top + plotH / 2)} y={20} textAnchor="middle" fontSize="14" fontWeight="bold" fill="#212529">{this.props.intl.formatMessage({ id: "IDS_DISTINCTSUBJECTCOUNT" })}</text>
//         </svg>
//       </div>
//     </div>
//   );
...
const ColumnsSVG = ({ title, categories = [], series = [], stacked = true, intl }) => {
  // --- sizing & dynamic padding
  const vbW = 960, vbH = 440; // a little taller for safety
  const maxLabelLen = Math.max(0, ...categories.map(c => String(c ?? '').length));
  // vertical footprint consumed by rotated labels; tuned for ~-38deg rotation
  ...
  // truncate category labels but keep full value in <title>
        ...
        <svg width="100%" height="100%" viewBox={`0 0 ${vbW} ${vbH}`} preserveAspectRatio="xMidYMid meet" role="img" aria-label={title}>
          ...
          {categories.map((cat, i) => {
            ...
            if (stacked) {
              ...
              return (
                <g key={`cat-${i}`}>
                  {/* rotated, truncated category label with tooltip (moved up slightly) */}
...
class BiobankECatalogueExternalModal extends React.Component {
  constructor(props) {
    super(props);
  ...
  onCatalogueSubmit = () => {
    ...
    const siteOption = selectedRecord.nsitecode ||
      (Array.isArray(this.state.projectSiteList) && this.state.projectSiteList.length ? this.state.projectSiteList[0] : null);

    const nbioprojectcode = this.getIdFromOption(projOption, 'nbioprojectcode');
    const nsitecode = this.getIdFromOption(siteOption, 'nsitecode');
    const userinfo = this.props.Login.userInfo;

    if (nbioprojectcode <= 0) { toast.info(this.props.intl.formatMessage({ id: "IDS_SELECTBIOPROJECT" })); return; }
    if (nsitecode <= 0) { toast.info(this.props.intl.formatMessage({ id: "IDS_SELECTSITE" })); return; }

    this.props.dispatch(initRequest(true));
    rsapi()
      .post("biobankecatalogueexternal/getAggregatedDataForCatalogue", {
  ...
  openView = (row) => {
    ...
    this.props.dispatch(initRequest(true));
    rsapi()
      .post("biobankecatalogueexternal/getDetailedDataForCatalogue", {
        nselectedprojectcode: nbioprojectcode,
        nselectedsitecode: nsitecode,
  ...
  onComboChange = (comboData, fieldName) => {
    ...
    if (fieldName === "nbioprojectcode") {
      ...
      rsapi()
        .post("biobankecatalogueexternal/getSiteComboForProject", {
  ...
  render() {
    const showExternalCatalogueModal = !!this.props.show;
    ...
    const detailColumns = [
      ...
      { idsName: 'IDS_VOLUMEµL', dataField: 'sqty', width: '120px', filter: 'numeric' },
    ...
    return (
      <Modal
        show={showExternalCatalogueModal}
            ...
            <Row className="mb-3">
              ...
              <Col md={4}>
                <FormSelectSearch
                  ...
                  placeholder={this.props.intl.formatMessage({ id: "IDS_SELECTRECORD" })}
                  isMandatory={true}
                  options={this.state.projectSiteList}
                  optionId='nsitecode'
                  optionValue='ssitename'
                  value={this.state.selectedRecord?.nsitecode || (Array.isArray(this.state.projectSiteList) && this.state.projectSiteList[0])}
...
export default injectIntl(connect(mapStateToProps)(BiobankECatalogueExternalModal));
