...
class AddBiobankECatalogueExternalRequest extends React.Component {
  emitUp = () => {
    try {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      isExistingRequest: !!(selectedRequest && selectedRequest.nbiobankecatreqexternalcode),
  ...
  componentDidUpdate(prevProps) {
    ...
    // BIO: take availablecount from Redux
    if (isBio && md.availablecount !== undefined && md.availablecount !== this.state.availablecount) {
      ...
      // push consolidated form data up to parent
      this.emitUp();
    }

    // PARENT: when the list refreshes, sync count from the selected code
    ...
    // Refresh local cached lists from props (unchanged logic)
    ...
    if (this.props.projectSiteList !== prevProps.projectSiteList) {
      ...
      // If no site selected yet and we received a list, pick the first and fetch types
      const hasNoSite = !this.state.selectedRecord?.nsitecode;
      const listNow = this.props.projectSiteList || [];
      if (hasNoSite && listNow.length > 0) {
        this.setState({ projectSiteList: listNow }, this.pickFirstSiteAndFetch);
        return;
      }
    }
  }
  ...
  // Select first site option (if any) and fetch BioSample types for that selection
  pickFirstSiteAndFetch = () => {
    const { projectSiteList = [] } = this.state || {};
    const firstSite = projectSiteList[0] || null;
    const selectedRecord = { ...(this.state.selectedRecord || {}) };

    if (!firstSite) {
      this.setState({
        selectedRecord: { ...selectedRecord, nsitecode: null, nproductcode: null },
        sampleTypeList: [],
        parentSamplesList: [],
        availablecount: 0,
    ...
    // set first site and clear dependents, then fetch types
    this.setState({
      selectedRecord: { ...selectedRecord, nsitecode: firstSite, nproductcode: null },
      sampleTypeList: [],
      parentSamplesList: [],
      availablecount: 0,
      currentRow: { sreqminvolume: '', nproductcode: null, sparentsamplecode: null, nreqnoofsamples: '' }
    }, this.loadBioSampleTypesForSelection);
  };

  // Fetch BioSample types for current project + site
  loadBioSampleTypesForSelection = () => {
    const userinfo = this.props?.Login?.userInfo || {};
    const nproject = this.getIdFromOption(this.state?.selectedRecord?.nbioprojectcode, 'nbioprojectcode');
    const nsite    = this.getIdFromOption(this.state?.selectedRecord?.nsitecode, 'nsitecode');

    if (!(nproject > 0 && nsite > 0)) return;

    this.props.dispatch(initRequest(true));
    rsapi().post('biobankecatalogueexternal/getBioSampleTypeCombo', {
  ...
  // Extracts the backend value/id from a UI option (no hard-coded labels)
  getReqTypeValue = (rf) => {
  ...
  applyComboChange = (comboData, fieldName, forceClearDependents = false) => {
    ...
    // Project change -> fetch sites (clear grid)
    if (fieldName === 'nbioprojectcode') {
      const userinfo = this.props.Login.userInfo;

      const clearThenFetchSites = () => {
        this.props.dispatch(initRequest(true));
        const nproject = this.getIdFromOption(comboData, 'nbioprojectcode');
        rsapi()
          .post('biobankecatalogueexternal/getSiteComboForProject', { nselectedprojectcode: nproject, userinfo })
          .then(response => {
            this.props.dispatch({ type: DEFAULT_RETURN, payload: { loading: false, shouldRender: false } });
            const sites = response.data?.lstSite || [];
            const projectSiteList = sites.map(s => ({
              label: s.ssitename, value: s.nsitecode, item: s, nsitecode: s.nsitecode
            }));

            selectedRecord.nsitecode = null;
            ...
            this.setState({
              ...
              availablecount: 0,
              currentRow: { sreqminvolume: '', nproductcode: null, sparentsamplecode: null, nreqnoofsamples: '' },
              request: { ...(this.state.request || {}), details: [] } // grid cleared on project change
            }, () => {
              this.props.onChange && this.props.onChange(this.state.request, this.state.request.details || []);
              // Auto-pick first site and fetch BioSample types
              this.pickFirstSiteAndFetch();
            });
          })
    ...
    // Site change -> fetch products (clear grid)
    if (fieldName === 'nsitecode') {
      const fetchProducts = () => {
        const nproject = this.getIdFromOption(selectedRecord.nbioprojectcode, 'nbioprojectcode');
        const nsite = this.getIdFromOption(comboData, 'nsitecode');
        if (nproject > 0 && nsite > 0) {
          const userinfo = this.props.Login.userInfo;
          this.props.dispatch(initRequest(true));
          rsapi()
            .post('biobankecatalogueexternal/getBioSampleTypeCombo', {
              nselectedprojectcode: nproject, nselectedsitecode: nsite, userinfo
            })
            .then(response => {
              this.props.dispatch({ type: DEFAULT_RETURN, payload: { loading: false, shouldRender: false } });
              ...
              this.setState(prev => ({
                ...
                availablecount: 0,
                currentRow: { ...prev.currentRow, sparentsamplecode: null, nreqnoofsamples: '' },
                request: { ...(prev.request || {}), details: [] } // grid cleared on site change
              }), () => {
                this.props.onChange && this.props.onChange(this.state.request, this.state.request.details || []);
              // Auto-pick first site and fetch BioSample types
              this.pickFirstSiteAndFetch();
      ...
      if (forceClearDependents) {
        this.setState({
          ...
          request: {
            ...(this.state.request || {}),
            nbioprojectcode: this.getIdFromOption(selectedRecord.nbioprojectcode, 'nbioprojectcode'),
            nsitecode: this.getIdFromOption(comboData, 'nsitecode'),
            projectType: selectedRecord.nbioprojectcode || null,
            receivingSite: comboData || null,
            sremarks: selectedRecord.sremarks ?? (this.state.request && this.state.request.sremarks),
            details: []
          }
        }, fetchProducts);
    ...
    // Product change -> DO NOT clear grid; just reset parent list/count and row inputs
    if (fieldName === 'nproductcode') {
      this.setState(prev => ({
        ...
        availablecount: 0,
        currentRow: { ...prev.currentRow, sparentsamplecode: null, nreqnoofsamples: '' }
        // request.details is intentionally preserved so existing rows remain
      }), () => this.resetAvailableCountRedux());
      return;
    }


    // Request form type change -> clear all dependent state and grid
  ...
  // decimal input guard so the custom operator can be typed freely
  ...
  onFilterAvailability = () => {
    ...
    if (!selectedRecord.nsitecode) { toast.info(this.props.intl.formatMessage({ id: 'IDS_SELECT' }) + " " + this.props.intl.formatMessage({ id: 'IDS_RECEIVINGSITE' })); return; }
    ...
    if (reqType === RequestFormType.BIO_SAMPLE_BASED) {
      ...
      // Unknown request form type; no fetch dispatched.
    }
  };
  ...
  onAddSample = () => {
    ...
    // Determine request type flags early
    const reqTypeFlags = (() => {
    ...
    const detail = {
      nbiobankecatreqexternaldetailcode: -1,
    ...
    return (
      <div>
        <Row>
          <Col md={4}>
            <FormSelectSearch
              formLabel={this.props.intl.formatMessage({ id: 'IDS_REQUESTFORMTYPE' })}
              isSearchable
              isDisabled={false}
              isMandatory={true}
              name={'nrequestformtypecode'}
              options={this.props.requestFormList}
              value={selectedRecord.nrequestformtypecode}
              onChange={(opt) => this.onComboChange(opt, 'nrequestformtypecode')}
            />
          ...
          <Col md={4}>
            <FormSelectSearch
              formLabel={this.props.intl.formatMessage({ id: 'IDS_RECEIVINGSITE' })}
              isSearchable
              isDisabled={true}
              isMandatory={true}
              name={'nsitecode'}
              ...
              onChange={(opt) => this.onComboChange(opt, 'nsitecode')}
            />
          </Col>
        </Row>
        ...
        <Row className="mt-2">
          ...
          <Col md={5}>
            <FormInput
              ...
              // onKeyDown={this.handleMinVolKeyDown}
        ...
        <Row className="mt-2 align-items-center">
          ...
          <Col md={6} className="d-flex justify-content-end">
            <FormInput
              ...
              isMandatory={true}
            />
            <Button className="ml-2" style={{ width: "3rem", height: "2.3rem" }} onClick={this.onAddSample}>{this.props.intl.formatMessage({ id: 'IDS_ADD' })}</Button>
          </Col>
        </Row>
            ...
            <Table bordered>
              <thead>
                <tr>
                  ...
                  <th>{this.props.intl.formatMessage({ id: 'IDS_PARENTSAMPLECODE' })}</th>
                ...
                {this.state.request.details && this.state.request.details.length > 0 ? (
                  this.state.request.details.map((d, i) => (
                    <tr key={i}>
                      ...
                      <td>{d.sparentSampleCode || d.sparentsamplecode || '-'}</td>
  ...
  getIdFromOption = (opt, key) => {
    ...
    if (typeof opt === "object") {
      ...
      const keys = ["nreqformtypecode", "nbioprojectcode", "nsitecode", "value", "id", "code"];
      for (const k of keys) {
        if (opt[k] != null) {
  ...
  hydrateDefaultsFromMasterData = () => {
    ...
    const siteOpt = selSite ? projectSiteList.find(o => this.getIdFromOption(o, "nsitecode") === selSite.nsitecode) : null;
    ...
    if (!selectedRecord.nsitecode && (siteOpt || selSite)) selectedRecord.nsitecode = siteOpt || selSite;

    this.setState({ selectedRecord });
  };
}

export default connect(mapStateToProps)(injectIntl(AddBiobankECatalogueExternalRequest));