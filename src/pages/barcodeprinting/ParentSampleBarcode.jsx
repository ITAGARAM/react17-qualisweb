...
import { callService, updateStore, crudMaster, getBarcodePrinter, getParentSampleBarcodedata, printBarcodes } from '../../actions';
...
class ParentSampleBarcode extends React.Component {
    ...
    render() {
        ...
        const parentSampleBarcode = this.state.controlMap.has("ParentSampleBarcode") && this.state.controlMap.get("ParentSampleBarcode").ncontrolcode;
        let extractedColumnList = []
        // extractedColumnList.push({ "idsName": "IDS_SAMPLEID", "dataField": "npkid", "width": "150px" })
        ...
        extractedColumnList.push({ "idsName": "IDS_PARENTSAMPLECODE", "dataField": "sparentsamplecode", "width": "200px" })
        return (
            <>
                <Row>
                    <Col>
                        <ListWrapper className="client-list-content">
                                ...
                                <Col>
                                    ...
                                    <div style={{ float: "right", marginRight: "1rem" }}>
                                        <Nav.Link className="btn btn-circle outline-grey mr-2"
                                            hidden={this.state.userRoleControlRights.indexOf(parentSampleBarcode) === -1}
                                            data-tooltip-id="my-tooltip" data-tooltip-content={this.props.intl.formatMessage({ id: "IDS_PRINTBARCODE" })}
                                             ...
                                             onClick={() => this.props.getBarcodePrinter({
                                                ...
                                                ncontrolcode: parentSampleBarcode,
                                                userInfo: this.props.Login.userInfo,
                                                control: "parentSampleBarcode",
                            ...
                            {this.state.data ?
                                <DataGridWithSelection
                                    ...
                                    userInfo={this.props.Login.userInfo}
                                    data={this.state.ParentSampleBarcodeData || []}
                                    ...
                                    dataResult={process(this.state.ParentSampleBarcodeData || [], this.state.dataState
                                        ? this.state.dataState : { skip: 0, take: 10 })}
    ...
    reloadData = () => {
        this.props.getParentSampleBarcodedata({
                    ...
                    if (this.state?.selectedRecord?.ncount !== undefined &&
                        ...
                        const inputParam = {
                            ...
                            inputData: {
                                ...
                                // nbioparentsamplecode: this.state?.dataResult?.data
                                //     ?.filter(x => this.state?.selectedSampleData?.some(item => item.npkid === x.npkid))
                                //     ?.map(x => x.npkid)?.join(", ") || " ",
                                sparentsamplecode: this.state?.dataResult?.data
                                    ?.filter(x => this.state?.selectedSampleData?.some(item => item.npkid === x.npkid))
                                    ?.map(x => `'${x.sparentsamplecode}'`)
                                    ?.join(",") || " ",
                                userinfo: this.props.Login.userInfo,
    ...
    dataStateChangeWorklistSample = (event) => {
        let updatedList = [];
        let ParentSampleBarcodeData = this.state.ParentSampleBarcodeData || [];
        this.setState({
            dataResult: process(this.state.ParentSampleBarcodeData || [], event.dataState),
            dataState: event.dataState, ParentSampleBarcodeData, addSelectAll: event.dataState && event.dataState.filter === null ?
                this.valiateCheckAll(ParentSampleBarcodeData) :
                this.valiateCheckAll(process(ParentSampleBarcodeData || [], event.dataState).data)
        });
    }
    ...
    headerSelectionChange = (event) => {
        ...
        let ParentSampleBarcodeData = this.state.ParentSampleBarcodeData || [];
        if (checked) {
            ParentSampleBarcodeData = ParentSampleBarcodeData.map(item => {
            ...
            this.setState({
                ParentSampleBarcodeData, selectedSampleData,
                addSelectAll: this.valiateCheckAll(process(ParentSampleBarcodeData || [], this.state.dataState).data),
            ...
            ParentSampleBarcodeData = ParentSampleBarcodeData.map(x => {
                const matchedItem = eventData.find(item => x.npkid === item.npkid);
                if (matchedItem) {
                    ParentSampleBarcodeData = ParentSampleBarcodeData.filter(item1 => item1.npkid !== matchedItem.npkid);
            ...
            this.setState({
                ParentSampleBarcodeData, selectedSampleData, deletedList: deletedListdData,
    ...
    selectionChange = (event) => {
        ...
        ParentSampleBarcodeData = ParentSampleBarcodeData.map(item => {
            if (item.npkid === event.dataItem.npkid) {
                ...
                if (item.selected) {
                    ...
                    ParentSampleBarcodeData = ParentSampleBarcodeData.filter(item1 => item1.npkid !== item.npkid)
        ...
        this.setState({
            addSelectAll: this.valiateCheckAll(process(ParentSampleBarcodeData || [], this.state.dataState).data), ParentSampleBarcodeData, selectedSampleData,
            deleteSelectAll: this.valiateCheckAll(ParentSampleBarcodeData)
        });
    }
    ...
    componentDidUpdate(previousProps) {
        if (this.props.Login.masterData !== previousProps.Login.masterData) {
            ...
            if (this.props.Login.userInfo.nformcode !== previousProps.Login.userInfo.nformcode) {
                ...
                this.setState({
                    ...
                    //ALPD-2252
                    dataResult: process(this.props.Login.masterData.ParentSampleBarcodeData || [], this.state.dataState),
                ...
                this.setState({
                    ...
                    dataResult: process(this.props.Login.masterData.ParentSampleBarcodeData ? this.props.Login.masterData.ParentSampleBarcodeData : [], dataState),
        ...
        if (this.props.Login.masterData.ParentSampleBarcodeData !== previousProps.Login.masterData.ParentSampleBarcodeData) {
            this.setState({
                ParentSampleBarcodeData: this.props.Login.masterData.ParentSampleBarcodeData
            });
        }
...
export default connect(mapStateToProps, { callService, updateStore, crudMaster, getBarcodePrinter, getParentSampleBarcodedata, printBarcodes })(injectIntl(ParentSampleBarcode));